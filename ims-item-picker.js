...
function movementAdapter(){
  ...
  return{
    ...
    anchor:$('bmLines'),
    context(){
      const action=$('bmAction')?.value||'',src=$('bmSource'),o=selectedOption(src);
      return{action,sourceId:src?.value||'',sourceName:o?.dataset?.name||o?.textContent?.trim()||''};
    },
    ready(c){return!!c.sourceId;},
    eligible(i,c){
      const map={RECEIVE_SUPPLIER:['supplier','At Supplier'],TRANSFER_WAREHOUSE:['warehouse','Available'],DELIVER_CLIENT:['warehouse','Available'],RETURN_CLIENT:['client','At Client']};
      const rule=map[c.action];if(!rule)return false;
      return balances(i).some(b=>sameLoc(b,c.sourceId,c.sourceName)&&b.locationType===rule[0]&&b.status===rule[1]);
    },
    selected(){return new Set([...document.querySelectorAll('.bmLine')].map(r=>r.dataset.itemId).filter(Boolean));},
    async add(i){
      const field=$('bmLookupField'),input=$('bmLookupValue'),btn=$('bmFindItem');if(!field||!input||!btn)return false;
      const before=document.querySelectorAll('.bmLine').length;
      field.value=i.alias?'alias':'name';input.value=i.alias||i.name||'';btn.click();
      return waitFor(()=>document.querySelectorAll('.bmLine').length>before,1800);
    }
  };
}
...
function serviceAdapter(){
  ...
  return{
    ...
    ready(c){return!!c.sourceId;},
    eligible(i,c){return balances(i).some(b=>{
    ...
    selected(){return new Set([...document.querySelectorAll('.scLine')].map(r=>r.dataset.id).filter(Boolean));},
    async add(i){
      const field=$('scLookupField'),input=$('scLookupValue'),btn=$('scFind');if(!field||!input||!btn)return false;
      const before=document.querySelectorAll('.scLine').length;
      field.value=i.alias?'alias':'name';input.value=i.alias||i.name||'';btn.click();
      return waitFor(()=>document.querySelectorAll('.scLine').length>before,1800);
    }
  };
}
...
function reservationAdapter(){
  ...
  return{
    ...
    ready(c){return!!c.key;},
    eligible(i,c){return!i.activeReservation&&balances(i).some(b=>b.status==='Available'&&`${b.locationType||''}|${b.locationId||''}|${b.locationName||''}`===c.key);},
    selected(){return new Set([...document.querySelectorAll('.resRow')].map(r=>r.dataset.itemId).filter(Boolean));},
    async add(i){
      let row=[...document.querySelectorAll('.resRow')].find(r=>!r.dataset.itemId);
      if(!row){$('resAddRow')?.click();row=[...document.querySelectorAll('.resRow')].at(-1);}
      if(!row)return false;
      const field=row.querySelector('.resLookupField'),input=row.querySelector('.resLookupValue'),btn=row.querySelector('.resFindItem');
      if(!field||!input||!btn)return false;
      field.value=i.alias?'alias':'name';input.value=i.alias||i.name||'';btn.click();
      return waitFor(()=>!!row.dataset.itemId,1800);
    }
  };
}
...
async function fetchPage(a,reset=false){
  ...
  try{
    ...
    while(out.length<PAGE_SIZE&&!exhausted&&guard<20){
      guard++;
      const base=[collection(db,'inventory'),orderBy(documentId(),'asc')];
      const q=cursor?query(...base,startAfter(cursor),limit(SCAN_SIZE)):query(...base,limit(SCAN_SIZE));
      const snap=await getDocs(q);
      if(!snap.size){exhausted=true;break;}
      for(const d of snap.docs){
        lastScanned=d;const item={id:d.id,...d.data()};
        if(a.eligible(item,ctx)&&!selected.has(item.id)){out.push(item);if(out.length===PAGE_SIZE)break;}
      }
      cursor=snap.docs.at(-1)||cursor;
      if(snap.size<SCAN_SIZE)exhausted=true;
      if(out.length===PAGE_SIZE)break;
    }
    s.items=out;s.next=!exhausted&&lastScanned?lastScanned:null;
    render(a,out.length?'':'No eligible items found on this page.');
...
function ensurePanel(a){
  ...
  panel.innerHTML=`<div class="flex flex-wrap items-center justify-between gap-2"><div><div class="font-semibold text-sm text-cyan-300">Browse Eligible Items</div><div class="text-[11px] text-slate-500">20 eligible items per page. Fast Pick by Wellora SN / R2R SN or Description remains available above.</div></div><button type="button" data-picker-refresh class="bg-slate-700 hover:bg-slate-600 px-3 py-2 rounded-lg text-xs">Refresh</button></div><div data-picker-body class="text-xs text-slate-500">Select the required source/location first.</div>`;
  a.anchor.parentElement?.insertBefore(panel,a.anchor);
  panel.querySelector('[data-picker-refresh]').onclick=()=>fetchPage(a,true);
  const watchIds=a.id==='movement'?['bmAction','bmSource']:a.id==='service'?['scSrcType','scSrc']:['resLocation'];
...
function render(a,message=''){
  ...
  body.querySelector('[data-picker-add]').onclick=async e=>{
    const ids=[...body.querySelectorAll('.imsPickerCheck:checked')].map(x=>x.value);if(!ids.length)return;
    const btn=e.currentTarget;btn.disabled=true;btn.textContent='Adding…';
    for(const id of ids){const item=s.items.find(x=>x.id===id);if(item)await a.add(item);}
    btn.disabled=false;btn.textContent='Add Selected';await fetchPage(a,true);
  };
}

function scan(){[movementAdapter(),serviceAdapter(),reservationAdapter()].filter(Boolean).forEach(ensurePanel);}
